...
const ChatHeader: React.FC<ChatHeaderProps> = ({
  ...
  const { location, getCurrentLocation, isLoading, error, permission } =
    useGeolocation()
  ...
  React.useEffect(() => {
    ...
    console.log(
      `🔄 [${timestamp}] ChatHeader: Props - onClose:`,
      !!onClose,
      'onClear:',
      !!onClear,
      'chatSession:',
  ...
  React.useEffect(() => {
    ...
    console.log(
      `🚀 [${timestamp}] ChatHeader: ChatSession:`,
      !!chatSession,
      'Location:',
      location,
      'Permission:',
    ...
    if (locationChanged) {
      console.log(
        `📍 [${timestamp}] ChatHeader: Location changed, updating chat session:`,
        location
      )
      console.log(
        `📍 [${timestamp}] ChatHeader: Previous:`,
        previousLocation.current,
        'New:',
        location
      )
    ...
    if (
      ...
      console.log(
        `🚀 [${timestamp}] ChatHeader: Automatically requesting location...`
      )
      ...
      getCurrentLocation()
        .then(detectedLocation => {
          const timestamp = new Date().toISOString()
          console.log(
            `🚀 [${timestamp}] ChatHeader: Auto-location successful:`,
            detectedLocation
          )
        ...
        .catch(error => {
          const timestamp = new Date().toISOString()
          console.log(
            `🚀 [${timestamp}] ChatHeader: Auto-location failed:`,
            error.message
          )
        })
    } else if (location) {
      console.log(
        `🚀 [${timestamp}] ChatHeader: Location already set, no action needed`
      )
    } else {
      console.log(
        `🚀 [${timestamp}] ChatHeader: Skipping auto-location - conditions not met`
      )
    }
  }, [chatSession, location?.lat, location?.lng, permission]) // Depend on primitive values to avoid reference equality issues

  return (
    <div className="bg-red-600 text-white px-4 py-3 md:px-6 md:py-4 rounded-t-2xl shadow-sm">
      <div className="flex items-center justify-between">
        {/* Left Section: Text Only */}
        <div>
          <h3 className="font-semibold text-lg leading-tight">McD Assistant</h3>
        </div>
        ...
        <div className="flex items-center gap-1 md:gap-2">
          ...
          <LocationButton
            ...
            size="small"
            showStatus={true}
            className="text-white hover:bg-red-700 focus:ring-red-300 border border-red-500 rounded-lg transition-colors min-h-[44px] md:min-h-auto"
          />

          {/* Close Button */}
          <button
            onClick={onClose}
            className="p-3 md:p-2 hover:bg-red-700 rounded-lg transition-colors min-w-[44px] min-h-[44px] md:min-w-auto md:min-h-auto flex items-center justify-center"
            title="Close chat"
          >
            <svg
